          ...
          <tr>
            {columns.map((column) => (
              <th
                key={column.id}
                scope="col"
                className={`${column.sticky ? `sticky-col sticky-col-${column.sticky}` : ""} ${column.className ?? ""}`}
              >
                {column.header}
              </th>
            ))}
            ...
            ? Array.from({ length: 8 }, (_, index) => (
                <tr key={`skeleton-${index}`} aria-hidden>
                  {columns.map((column) => (
                    <td
                      key={column.id}
                      className={column.sticky ? `sticky-col sticky-col-${column.sticky}` : undefined}
                    >
                  ...
                  <Fragment key={id}>
                    ...
                    {expanded && renderExpanded ? (
                      <tr className="bg-white">
                        <td colSpan={columns.length} className="whitespace-normal p-3">
                          {renderExpanded(row)}
                        </td>
                      </tr>
                    ) : null}

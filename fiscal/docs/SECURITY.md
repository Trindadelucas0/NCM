...
- RBAC `admin` | `consulta` nas rotas (não só no menu).
- Tenant: `companyId` em toda query de negócio; IDOR → 404.
...
- Segredos só em `.env` (gitignored). `.env.example` sem senha real. Seed usa `SEED_ADMIN_PASSWORD`.
